...
dynamic_phenomenon ltrip(V,PA,PB) =: [end(moored(V,PA)), underway(V), start(moored(V,PB))].
dynamic_phenomenon lfishing_trip(V,PA,FA,PB) =: [end(moored(V,PA)), underway(V),in_fishing_area(V,FA), start(moored(V,PB))].
...
dynamic_phenomenon trip_f(V,PA,FA,PB):=
    trip(V,PA,PB) contains in_fishing_area(V,FA).

%dynamic_phenomenon ucfa(V,FA):=
    %underway(V) contains in_fishing_area(V,FA).
