...
test(query1):-
    ...
    recognition_query(7,5,10),
    assertion(findall(_,(input_event_instant(_,T),T=<3),[])),
    assertion(findall(_,(input_state_interval(_,[_,Te]),Te=<3),[])).
...
test(case9):-
    assertion(findall(L,state_intervals(happy(bob),L),[[[1,5],[6,8]]])),
    assertion(findall(L,state_intervals(happy(alice),L),[[[4,inf]]])),
...
test(case12):-
    assertion(findall(L,state_intervals(happy_without_money(bob),L),[[[1,5]]])),
